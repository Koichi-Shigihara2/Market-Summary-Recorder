...
{news_section}
"""
    # gemini-1.5-flash は2025年に廃止。利用可能なモデルを優先順位付きで試行する。
    model_names = [
        "gemini-2.0-flash",
    ...
    for model_name in model_names:
        try:
            print(f"[INFO] モデル試行中: {model_name}")
            response = client.models.generate_content(
                model=model_name,
                contents=prompt
            )
            print(f"[INFO] モデル成功: {model_name}")
            return response.text
        except Exception as e:
            print(f"[WARN] モデル失敗 ({model_name}): {e}")
            last_error = e
    print(f"Gemini API エラー: すべてのモデルで失敗しました")
    raise last_error
