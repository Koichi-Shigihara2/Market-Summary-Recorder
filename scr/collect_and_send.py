...
def get_realtime_data():
    """主要指標・需給・セクター・クレジットを取得。
    戻り値: (表示用テキスト, 構造化データの辞書)
    """
    summary = ""
    data = {}
    ...
    if nya_hist is not None and len(nya_hist) >= 2:
        ...
        if nya_pct < 0 and vol_ratio > 1.1:
            summary += "  出来高増＋下落：ディストリビューション（大口売り抜け）の疑い\n"
        elif nya_pct > 0 and vol_ratio > 1.1:
            summary += "  出来高増＋上昇：買い需要が旺盛\n"
...
def extract_judgment(report_text):
    """Geminiレポートから「判定：晴れ/曇り/嵐」を抽出する"""
    match = re.search(r'判定：([晴曇嵐]+)', report_text)
    if match:
